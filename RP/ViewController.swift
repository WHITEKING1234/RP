...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func computerChoice() -> String {
        ...
        if selectedDifficulty == 3 {
            // Easy Mode: Случайный выбор
            return option.randomElement() ?? "Rock"
        } else {
            // Hard Mode: Создаём случайный выигрышный выбор для компьютера
            ...
            // Найдем выигрышный вариант для компьютера
            for case (let player, let computer) in winCases {
                if playerChoice == player {
                    return computer
                }
            }
            
            // Если не нашлось, выбираем случайный вариант
            return option.randomElement() ?? "Rock"
        }
    }
    ...
    func winLogic() {
        ...
        if playerChoice == computerChoice {
            ...
            computerScore += 1 // Увеличить счёт компьютера
        }
